...
def bulk_phonon_dispersion(N):
    ...
    dispersion[:,1]=[abs(1369.42*k-2.405e-8*(k**2)-9.70e-19*(k**3)) for k in dispersion[:,0]]
    dispersion[:,2]=[abs(1081.74*k-7.711e-8*(k**2)+5.674e-19*(k**3)+7.967e-29*(k**4)) for k in dispersion[:,0]]
    dispersion[:,3]=dispersion[:,2]
    return dispersion
...
def internal_scattering_time_calculation(frequency, polarization):
    ...
    # time_of_internal_scattering=tau_internal
...
def output_distributions():
    ...
    free_path_distribution=distribution_calculation("All free paths.txt", length, number_of_nodes)
    fig, ax = plt.subplots()
...
def output_information(start_time, simulation_scheme):
    ...
    print ("\n", percentage, '% of phonons reached the end of the system')
