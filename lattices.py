...
def hole_positioning(hole_lattice_type, rectangular_hole_side_y, rectangular_hole_side_x, width, period_x, period_y):
    ...
    if hole_lattice_type=='sources':
        ...
        number_of_periods_y=3
        number_of_sources=2
        hole_coordinates=zeros((number_of_sources*(number_of_periods_x*number_of_periods_y*4 + number_of_periods_y*4),3))
        hole_shapes=['circle' for x in range(hole_coordinates.shape[0])]
        hole_number=0
        for k in range(number_of_sources):
            for i in range(number_of_periods_y):
                for j in range(number_of_periods_x):
                    # Aligned rows
                    hole_coordinates[hole_number,0]=j*period_x+period_x/2.0+(number_of_periods_x+0.5)*k*period_x
                    hole_coordinates[hole_number,1]=first_hole_coordinate+i*period_y*2
    ...
    if hole_lattice_type == 'tesla valve backward':
        ...
        for i in range(repetitions):
            ...
            hole_coordinates[i,1] = (i+1)*period_y/2 + first_hole_coordinate
            hole_shapes.append('triangle_up')
    ...
    if hole_lattice_type=='cloak_3':
        ...
        for j in range(2):
            for i in range(5):
                hole_coordinates[N,0]=sign(1-2*j)*((i+1)*period_x/2+period_x*cos(1.0*pi/6.0))
                hole_coordinates[N,1]=first_hole_coordinate+period_y*(i+1)-period_y*sin(1.0*pi/6.0)
                N+=1
            for i in range(4):
                hole_coordinates[N,0]=sign(1-2*j)*((i+3)*period_x/2+period_x*cos(1.0*pi/6.0))
                hole_coordinates[N,1]=first_hole_coordinate+period_y*(i+1)-period_y*sin(1.0*pi/6.0)
                N+=1
            for i in range(2):
                hole_coordinates[N,0]=sign(1-2*j)*((i+5)*period_x/2+period_x*cos(1.0*pi/6.0))
                hole_coordinates[N,1]=first_hole_coordinate+period_y*(i+1)-period_y*sin(1.0*pi/6.0)
                N+=1
