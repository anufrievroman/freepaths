# freepaths/interface_smmm.py 24/06
...
def lambda_from_k(k): #3/07
    """
    Compute the wavelength from the wavevector k.
    """
    return 2 * math.pi / k if k != 0 else float('inf')

def P_spec(roughness, wavelength):
    """
    Specularity according to the modified Ziman model.

    Parameters:
        roughness -- interface roughness (in meters)
        wavelength -- wavelength of the incident phonon (in meters)

    Returns:
        Specularity coefficient between 0 (diffuse) and 1 (specular)
    """
    return math.exp(-16 * math.pi**2 * roughness**2 / wavelength**2)
...
def alpha_total_2T(theta_i, vg_i, vg_j, rho_i, rho_j, omega_i, omega_j, material_i, material_j, branch_number, roughness):
    ...
    P_i = P_spec(roughness, wavelength_i)
    P_j = P_spec(roughness, wavelength_j)
